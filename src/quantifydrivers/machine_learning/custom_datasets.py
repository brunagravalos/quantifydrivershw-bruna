...
class LargeScale_Dataset_extremes(Dataset):
    def __init__(self, file_g500, file_g200, file_psl, start_date, end_date, months, start_lag, lags_era5, variables, valid_times, transform=None):
        ...
        for var_name in self.variables:
            arr = datasets[var_name][self.lagged_vars[var_name]].to_array().transpose('time', 'variable', 'lat',
                                                                                      'lon').values
...
class LocalScale_Dataset_extremes_location_swvl_averaged_including_CO2(Dataset):
    ...
    def __init__(self, file_path, file_CO2, start_date, end_date, months, variables, scaler=None):
        ...
        self.dsco2 = open_xr_dataset(file_CO2).sel(time=self.ds.time, method='nearest')
...
class SPEI_extremes_location_dataset(Dataset):
    ...
    def __init__(self, file_path, file_CO2, files_spei, start_date, end_date, months, spei_variables, num_lags,
                 scaler=None):
        """
        Args:
            file_path (str): Path to the NetCDF file containing the ERA5 land dataset.
