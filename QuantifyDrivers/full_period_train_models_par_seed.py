...
for site in sites: 
    ...
    train_dataset = functions_NN_extremes.ERA5LandDataset_extremes_location_swvl_averaged_including_CO2(file_path=f"/gpfs/scratch/bsc32/bsc167965/tfm_data/era5_land/lagged_anomalies_and_event_detection/std_changed_{site}_lagged_standarized_anomalies_and_extreme_detection.nc", file_CO2=file_CO2 , **_ERA5LAND_TRAIN_DATASET_CONF)    
    ...
    extreme_weights_ctt = 1.
    nonextreme_weight_ctt = 1.
    ...
    # Complete period datasets for SHAP -----------------------------------------------
    ...
    complete_combined_dataloader = DataLoader(complete_combined_dataset, **_DATALOADERS_TEST_CONF) # Important: not shuffle in the evaluation dataloader to keep the results ordered in time 

    # ---------------------------------------------------------------------------------
    
    
    reset_seeds(seed)
    y_true, y_pred, outputs_prob,extreme_acc, nonextreme_acc = functions_NN_extremes.evaluate_CombinedModel(CombinedModel=model,cnn=cnn_model_loaded,nn=nn_model_loaded, test_loader=complete_combined_dataloader, print_accuracies=True,train_alone=False)
    ...
    with open(os.path.join(main_path, f'{site}/FullPeriod_results_data_{seed}.pkl'), 'wb') as f:
    ...
    with open(f'/gpfs/scratch/bsc32/bsc167965/tfm_data/test_train_n_shap_dilation/{site}/SHAP/FullPeriod_SHAP_values_GradientExplainer_not_filtered_{seed}_{number_lags}lags.pkl', 'wb') as f:
