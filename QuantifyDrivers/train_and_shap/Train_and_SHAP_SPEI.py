...
# This scripts is similar to Train_and_SHAP.py, but now we include the SPEI or SPI as input to the NN instead of swvl[1,2,3]. 
# This is done to check the robustness of the method with E-OBS observational data, where we do not have the swvl variables, due to the lack of long time 
# series of soil moisture data. Instead, we compute SPEI and SPI as proxies for soil moisture conditions. 
...
def load_hypms_from_file(site_name, percentile='90p', base_path='/home/bsc/bsc167965/TFM/ML/HYPM_tunning_outputs',file_name=None):
    ...
    """
    hypms = {}
    file_path = os.path.join(base_path, file_name) if file_name else os.path.join(base_path, f"{site_name}_{percentile}_hypms.txt")
    
    if not os.path.exists(file_path):
        print(f"Warning: Hyperparameter file not found for {site_name} at {file_path}")
        raise FileNotFoundError(f"Hyperparameter file not found for {site_name} at {file_path}")
...
sites_to_load_hypms = sites  # Sites for which we want to load hyperparameters

SITE_HYPMS_fixed = {'belgrado':{'lr':1e-4,'w_decay':0.01,'batch_size':32},
             'hannover':{'lr':1e-4,'w_decay':0.01,'batch_size':32}, 
             'stockholm':{'lr':1e-4,'w_decay':0.01,'batch_size':32}, 
             'lyon':{'lr':1e-4,'w_decay':0.01,'batch_size':32}, 
             'cordoba':{'lr':1e-4,'w_decay':0.01,'batch_size':32}, 
             'marrakech':{'lr':1e-4,'w_decay':0.01,'batch_size':32}}



# Create empty dictionary with the base HYPMS
SITE_HYPMS = SITE_HYPMS_fixed.copy()  # Start with fixed hyperparameters
...
for site_name in sites_to_load_hypms:
...
# -----------------------------------------------------------------------------------------------------------
    ...
    test_features_era5 = functions_NN_extremes.ERA5Dataset_extremes(file_g500,file_g200,file_psl, **_ERA5_TEST_DATASET_CONF) # shape: features, time, lat, lon
    ...
    combined_test_dataset = functions_NN_extremes.CombinedDataset(test_dataset,test_features_era5, variables = ['g500','g200', 'psl'])
    ...
    # ==============================================================================================================
    # Finished doing configuration. Final training of the combined model -------------------------------------------------------------------------------------------------------------------------------------------------
    # ==============================================================================================================
    ...
    reset_seeds(seed)
    # Optimizer for the combined model
    ...
    losses_train_combined, losses_val_combined, num_e, best_val_loss = functions_NN_extremes.train_CombinedModel(model,combined_train_loader, combined_val_loader, criterion=criterion,
    ...
    torch.save(model.state_dict(), f"/your/path/to/save/weights/model/file_name.pth")
